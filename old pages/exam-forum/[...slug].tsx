...
const ExamForumPage = () => {
  ...
  const getContent = () => {
    ...
    const examForum = navItems['Exam Forum'];
    let content: any = examForum;
    ...
    pathParts.forEach((part, index) => {
      ...
      if (index === 0) {
        // First level (Recruitment Exams, Higher Edu Exams)
        content = Object.entries(examForum).find(([key]) => 
          key.toLowerCase().replace(/[^a-zA-Z0-9]/g, '-') === part
        )?.[1];
        title = normalizedPart;
      } else if (index === 1) {
        // Second level (Group A, Group B, etc.)
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-24">
        <div className="container mx-auto px-4 py-8">
          ...
          <p className="mt-4 text-gray-600">The requested Exam Forum page could not be found.</p>
        </div>
      </div>
    );
  }
        ...
        <nav className="text-sm mb-6">
          <ol className="list-none p-0 inline-flex">
            <li className="flex items-center">
              <a href="/exam-forum" className="text-blue-600 hover:text-blue-800">
                Exam Forum
              </a>
