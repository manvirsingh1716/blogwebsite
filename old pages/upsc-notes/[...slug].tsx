...
const UPSCNotesPage = () => {
  ...
  const getContent = () => {
    ...
    pathParts.forEach((part, index) => {
      ...
      if (index === 0) {
        // First level (GS1, GS2, etc.)
        content = Object.entries(upscNotes).find(([key]) => 
          key.toLowerCase().replace(/[^a-zA-Z0-9]/g, '-') === part
        )?.[1];
        title = normalizedPart;
      } else if (index === 1) {
        // Second level (History, Geography, etc.)
        content = Object.entries(content).find(([key]) =>
          key.toLowerCase().replace(/[^a-zA-Z0-9]/g, '-') === part
        )?.[1];
        title = normalizedPart;
      } else if (index === 2) {
        // Third level (specific topics)
        content = content.find((topic: string) =>
          topic.toLowerCase().replace(/[^a-zA-Z0-9]/g, '-') === part
        );
        title = normalizedPart;
      }
    });
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-24">
        <div className="container mx-auto px-4 py-8">
          ...
          <p className="mt-4 text-gray-600">The requested UPSC Notes page could not be found.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <nav className="text-sm mb-6">
          <ol className="list-none p-0 inline-flex">
            <li className="flex items-center">
              <a href="/upsc-notes" className="text-blue-600 hover:text-blue-800">
                UPSC Notes
              </a>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          {Array.isArray(content) ? (
            ...
            // Render navigation for subtopics
